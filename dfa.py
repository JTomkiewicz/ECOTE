...
class State:
    def __init__(self, alphabet, id_list, id, id_marker):
        self.id = id
        self.id_set = set(id_list)
        # transitions to other states
        self.transitions = dict()
        self.final = id_marker in self.id_set
        # stransitions by chars are stored in dictionaries
        for char in alphabet:
            self.transitions[char] = {}  # empty dictionary
...
class DFA:
    ...
    def build_dfa(self):
        first_state = State(self.alphabet, self.tree.root.firstpos,
                            self.sequence(), self.tree.count - 1)
        ...
        while len(temp_states) > 0:
            ...
            states_list = self.transition(first_state)
    ...
    def transition(self, state):
        ...
        for i in state.id_set:
            if i == self.tree.count - 1:
                continue
            label = self.tree.leaves[i]
            if bool(state.transitions[label]) == False:
                state.transitions[label] = self.tree.followpos[i]
            else:
                state.transitions[label] = state.transitions[label] | self.tree.followpos[i]
    ...
    def can_be_generated(self, string):
        ...
        for char in string:
            if char not in self.alphabet:
                return False
            if bool(self.states[state - 1].transitions[char]) == False:
                return False
